...
class AasbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('asb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `asb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM asb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO asb (date, title, link, content)"
